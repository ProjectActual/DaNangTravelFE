  ...
  <div class="row">
    ...
    <div class="col-sm-7">
      <div class="form-group col-sm-12">
        ...
        <input id="title" class="form-control" value="{{ $post['data']['post']['data']['attributes']['title'] }}" placeholder="Nhập nội dung tiêu đề tại đây">
      </div>
      <div class="form-group col-sm-12">
        <label>Link bài viết</label>
        <div class="input-group">
          <input id="link" class="form-control" value="{{ $post['data']['post']['data']['attributes']['uri_post'] }}" placeholder="Dữ liệu được sinh từ tiêu đề" disabled>
      ...
      <div class="form-group col-sm-12">
        ...
        <select id="danh_muc" class="form-control">
          @foreach($categories['data']['categories']['data'] as $category)
            <option value="{{ $category['id'] }}" {{ $category['id'] == $post['data']['post']['data']['attributes']['category_id'] ? 'selected' : '' }}>{{ $category['attributes']['name_category'] }}</option>
          @endforeach
        </select>
      </div>
      <div class="form-group col-sm-12">
        <label>Sơ lược bài viết</label>
        <textarea id="summary" class="form-control" placeholder="Nhập sơ lược bài viết tại đây" rows="3">{{ $post['data']['post']['data']['attributes']['summary'] }}</textarea>
      </div>
    ...
    <div class="col-sm-5">
      ...
      <div class="col-sm-12" style="padding: 5px 15px;">
        <input type="file" id="avt_post" name="avatar_post" data-default-file="{{ $post['data']['post']['data']['attributes']['avatar_post'] }}" value="{{ old('avatar_post') }}">
      </div>

      @php
        $arrTag = [];

        foreach($post['data']['post']['data']['attributes']['tag'] as $tag) {
          $arrTag[] = $tag['tag'];
        }
      ...
      <div class="form-group col-sm-12">
        <input id="checkbox" type="checkbox" class="checkboxes" {{ $post['data']['post']['data']['attributes']['status'] == "ACTIVE" ? 'checked' : '' }}>
    ...
    <div class="col-sm-12">
      ...
      <div class="col-sm-12">
        <textarea id="content" class="form-control" rows="30" placeholder="Nhập nội dung của bài viết">{{ $post['data']['post']['data']['attributes']['content'] }}</textarea>
      </div>
    </div>
